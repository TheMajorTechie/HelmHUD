...
class BLESimpleCentral:
    def __init__(self, ble, rxbuf=100):
        ...
        self._ble.config(mtu=100)
        self._ble.irq(self._irq)
        self._connections = set()
        
        self._rx_buffer = bytearray()
        ((self._tx_handle, self._rx_handle),) = self._ble.gatts_register_services((_UART_SERVICE,))
        self._ble.gatts_set_buffer(self._rx_handle, rxbuf, False)
        self._reset()
...
def hhuart_receiver():
    ...
    SENSOR_TYPE_GENARRAY_COMMANDS = ["poll", "pressure", "temp", "hum", "lux", "uvs", "gas", "voc", "acc", "gyr", "mag", "hr"]
    ready_next_command = True
    command_queue_index = 0
    commands_sent = 0
    ...
    while True:
        if ready_next_command is True:
            print("ready command")
            if central.is_connected() and ready_next_command:
