...
'''
#Modified by the HelmHUD team to fit the HelmHUD project

#import serial
import time
import smbus_wrapper as smbus
#import smbus #Original code line: replaced with smbus_wrapper
...
class struct_lat_lon:
  def __init__(self):
    ...
    self.lat_mm = 0
    self.lat_mmmmm = 0
    self.lat_direction = "Y"
    self.latitude_degree = 0.00
    self.latitude = 0.00
    self.lon_ddd = 0
    self.lon_mm = 0
    self.lon_mmmmm = 0
    self.lon_direction = "X"
    self.lonitude = 0.00
...
class DFRobot_GNSS(object):#This has been modified to only use I2C. dont call this, use the I2C-specific class below.
  ...
  def __init__(self, bus, Baud):
    lat_lon = struct_lat_lon() #NEW
    if bus == 0:
      self.i2cbus = smbus.SMBus(bus)
      self.__uart_i2c = I2C_MODE
    else:
      print("ERROR:UART MODE") 
    '''else:
  ...
  def get_lat(self):
    ...
    '''
    lat_lon = struct_lat_lon()#New line: resets lat_lon. original code does not.
  ...
  def rgb_off(self):
    '''!
      @brief Turn rgb off
    '''
    #print("my type is " + str(type(self.__txbuf)))#TODO: REMOVE. type of __txbuf is 'list'. 
...
class DFRobot_GNSS_I2C(DFRobot_GNSS): 
  ...
  def write_reg(self, reg, data):
    while 1:
      print ("trying to write addr::" + str(self.__addr) + ":register:" + str(reg) + ":data:"+ str(data))
      
      self.i2cbus.write_i2c_block_data(self.__addr, reg, data[0]) # added [0]
    ...
    """while 1:
      try:
        print ("trying to write::" + str(self.__addr) + "::" + str(reg) + "::"+ str(data))
        self.i2cbus.write_i2c_block_data(self.__addr, reg, data)
  ...
  def read_reg(self, reg, len):
    ...
    return rslt
#this has been commented out because our intended use is with I2C. 
